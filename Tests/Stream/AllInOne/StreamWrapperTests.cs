...
using Submitter = ArmoniK.Api.gRPC.V1.Submitter;
...
internal class StreamWrapperTests
{
  ...
  public void SetUp()
  {
    ...
    ////serviceCreator.CreatePollster();
    //serviceCreator.CreateSubmitter();
    ////serviceCreator.CreateWorker();
    //logger_ = serviceCreator.LoggerFactory.CreateLogger<StreamWrapperTests>();
    //client_ = serviceCreator.createSubmitterClient();
    //logger_.LogInformation("Client created");
    Log.Logger = new LoggerConfiguration().WriteTo.Console(new CompactJsonFormatter())
    ...
    submitterTestHelper_ = new SubmitterTestHelper(Log.Logger);
    channel_             = submitterTestHelper_.GrpcChannel;
    ...
    logger_       = loggerFactory.CreateLogger<StreamWrapperTests>();
  ...
  {
    channel_.Dispose();
    submitterTestHelper_.Dispose();
  }

  private SubmitterTestHelper         submitterTestHelper_;
  private GrpcChannel                 channel_;
  private Submitter.SubmitterClient   client_;
  private ILogger<StreamWrapperTests> logger_;
  ...
  public async Task<int> Square(int input)
  {
    ...
    logger_.LogInformation("Creating Session");
    ...
    logger_.LogInformation("Session Created");
    ...
    logger_.LogInformation("TaskRequest Created");
    ...
    switch (createTaskReply.DataCase)
    {
      ...
      case CreateTaskReply.DataOneofCase.Successfull:
        logger_.LogInformation("Task Created");
        break;
      default:
        throw new ArgumentOutOfRangeException();
    }
